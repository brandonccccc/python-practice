...
def add_file():
    files = filedialog.askopenfilenames(title="이미지 파일을 선택하세요.", \
        filetypes=(("PNG 파일", "*.png"), ("모든 파일", "*.*")), \
            initialdir="C:/Users/지윤/PycharmProjects/pythonProject/python-practice/pygame_project/images") # 최초에 보여줄 경로
...
def merge_image():
    ...
    image_sizes = []  #(width1, heght1), (width2, height2)...
    if img_width > -1:
    ...
    #계산식
    # 100 * 60 이미지 -> width를 80으로 줄이면 높이는?
    # 원본 너비 : 원본 높이 = 변경 너비 : 변경 높이
    #    100   :   60  =   80   :   ?
    ...
    for idx, img in enumerate(images):
        # 너비가 원본유지가 아님녀 이미지 크기 조정
        if img_width > -1:
            img = img.resize(image_sizes[idx])

        result_img.paste(img, (0, y_offset))
        y_offset += (img.size[1] + img_space)  # height + 사용자 지정 간격
