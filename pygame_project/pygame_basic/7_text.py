...
character_x_pos = (screen_width - character_width) / 2 #화면 가로의 절반에 위치  (가로위치)
...
enemy_x_pos = (screen_width - enemy_width) / 2 #화면 가로의 절반에 위치  (가로위치)
enemy_y_pos = (screen_height / 2) - (enemy_height / 2)  # 화면 세로크기 가장 아래에 해당하는 곳에 위치 (세로 위치)

# 폰트 정의
game_font = pygame.font.Font(None, 40) #폰테객체 샌성(폰트, 크기)
