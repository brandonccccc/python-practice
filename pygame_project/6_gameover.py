...
weapon_width = weapon_size[0] #캐릭터 중간에서 무기가 나갈 수 있도록 하려면 값을 알아야함
...
balls.append({
    ...
    "to_x" : 3, # 공의 x 축 이동 방향, -3이면 왼쪽, 3이면 오른쪽
    "to_y" : -6, # y축 이동 방향
...
#게임 종료 메세지 / time over(시간 초과 실패) ,mission complete(성공), game over(맞아서 실패)
    ...
    for ball_idx, ball_val in enumerate(balls):
        ...
        for weapon_idx, weapon_val in enumerate(weapons):
            ...
            if weapon_rect.colliderect(ball_rect):
                ...
                ball_to_remove = ball_idx      # 해당 공 없애기 위 한 값 설정
